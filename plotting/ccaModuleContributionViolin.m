...
violinplot(dat(dat>0), si(dat>0), 'violincolor', [ 0.1 0.2 0.7 ]);
set(gca, 'TickDir', 'out', 'XTickLabel', labels, 'XtickLabelRotation', 90);

set(gca, 'ylim', [ -0.06 0.08 ]);
saveas(fh1, fullfile('figs', 'violin', 'fig3d_pos'), 'epsc');

fh2 = figure;
set(gcf, 'renderer', 'painters');
violinplot(dat(dat<0), si(dat<0), 'violincolor', [ 0.7 0.1 0.1 ]);
set(gca, 'TickDir', 'out', 'XTickLabel', labels, 'XtickLabelRotation', 90);

set(gca, 'ylim', [ -0.06 0.08 ]);
saveas(fh2, fullfile('figs', 'violin', 'fig3d_neg'), 'epsc');

%% NOT A WORKING FUNCTION YET
